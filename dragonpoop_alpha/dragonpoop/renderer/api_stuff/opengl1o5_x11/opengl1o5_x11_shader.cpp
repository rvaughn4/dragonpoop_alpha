...
namespace dragonpoop
{
    ...
    //ctor
    opengl1o5_x11_shader::opengl1o5_x11_shader( render_api_context_writelock *c, dpmutex_master *mm, opengl1o5_x11_functions *gl ) : render_api_shader( c, mm )
    {
        this->gl = gl;
    }
    ...
    bool opengl1o5_x11_shader::render( render_api_context_writelock *ctx, render_api_texture_ref *t0, render_api_texture_ref *t1, render_api_indexbuffer_ref *ib, render_api_vertexbuffer_ref *vb, dpmatrix *m, float alpha, dpmatrix *m_world, float r, float g, float b )
    {
        ...
        unsigned int gltex0, gltex1, vbo;
        dpvertex_buffer *bvb;
        dpindex_buffer *bib;
        ...
        return this->_render( ctx, gltex0, gltex1, bib, bvb, m, alpha, m_world, r, g, b, vbo );
    }

    //render gl
    bool opengl1o5_x11_shader::_render( render_api_context_writelock *ctx, unsigned int t0, unsigned int t1, dpindex_buffer *ib, dpvertex_buffer *vb, dpmatrix *m, float alpha, dpmatrix *m_world, float r, float g, float b, unsigned int vbo )
    {
        return 1;
    }

    //render vb
    void opengl1o5_x11_shader::renderVB( dpvertex_buffer *vb, dpindex_buffer *ib, unsigned int vbo )
    {
        dpvertex *pv;//, *iv;
        dpindex *pi;//, *ii;
        ...
        if( sizeof( dpindex ) == sizeof( uint16_t ) )
        {
            ...
            this->gl->glEnableClientState( GL_NORMAL_ARRAY );
            this->gl->glEnableClientState( GL_TEXTURE_COORD_ARRAY );
            this->gl->glEnableClientState( GL_VERTEX_ARRAY );

            if( this->gl->bUseVB )
            {
                this->gl->glBindBuffer( GL_ARRAY_BUFFER, vbo );
                if( vbo && this->gl->glGetError() != 0 )
                {
                    vbo = 0;
            ...
            this->gl->glTexCoordPointer( 2, GL_FLOAT, sizeof( dpvertex ), &pv->texcoords[ 0 ] );
            this->gl->glNormalPointer( GL_FLOAT, sizeof( dpvertex ), &pv->normal );
            this->gl->glVertexPointer( 3, GL_FLOAT, sizeof( dpvertex ), &pv->pos );
            this->gl->glDrawElements( GL_TRIANGLES, ib->getSize(), GL_UNSIGNED_SHORT, pi );
