...
namespace dragonpoop
{
    ...
    renderer::renderer( core *c, gfx_writelock *g, dptaskpool_writelock *tp ) : shared_obj( c->getMutexMaster() )
    {
        ...
        this->tsk = new dptask( c->getMutexMaster(), this->gtsk, 14/*14*/, 1 );
        tp->addTask( this->tsk );
...
//            if( this->bActive )
  //              tskl->setDelay( 14 );
    ...
    void renderer::runModels( dpthread_lock *thd, renderer_writelock *rl )
    {
        ...
        gl = (gfx_readlock *)og.tryReadLock( this->g, 20 );
        if( !gl )
            return;
        ...
        for( ii = li.begin(); ii != li.end(); ++ii )
        {
            ...
            if( !p )
                continue;
            ppl = (renderer_model_writelock *)og.tryWriteLock( p, 20 );
            if( !ppl )
                continue;
            ppl->run( thd, pl );
        }
    ...
    void renderer::renderModels( dpthread_lock *thd, renderer_writelock *rl )
    {
        ...
        {
            p = *i;
            pl = (renderer_model_readlock *)o.tryReadLock( p, 10 );
            if( !pl )
                continue;
            pl->render( thd, rl );
        }
