...
namespace dragonpoop
{
    ...
    model_component *model_writelock::findComponent( uint16_t mtype, std::string *s )
    {
        return this->t->findComponent( mtype, s );
    }

    //find components
    void model_writelock::getComponents( std::list<model_component *> *l )
    {
    ...
    //add FrameJoint
    model_frame_joint *model_writelock::makeFrameJoint( dpid id, dpid frame_id, dpid joint_id )
    {
        return this->t->makeFrameJoint( id, frame_id, joint_id );
    }
    
    //find FrameJoint
    model_frame_joint *model_writelock::findFrameJoint( dpid id )
    {
        return this->t->findFrameJoint( id );
    }
    
    //get FrameJoints
    void model_writelock::getFrameJoints( std::list<model_frame_joint *> *l )
    {
        this->t->getFrameJoints( l );
    }
    
    //get FrameJoints by frame or animation id
    void model_writelock::getFrameJoints( std::list<model_frame_joint *> *l, dpid parent_id )
    {
        this->t->getFrameJoints( l, parent_id );
    }
    
    //get FrameJoints by frame and animation id
    void model_writelock::getFrameJoints( std::list<model_frame_joint *> *l, dpid parent_id_1, dpid parent_id_2 )
    {
        this->t->getFrameJoints( l, parent_id_1, parent_id_2 );
    }
    
    //write model header to file/memory
    bool model_writelock::writeHeader( dpbuffer *b )
    {
    ...
    //get model dimensions
    void model_writelock::getCenter( dpxyz_f *x )
    {
