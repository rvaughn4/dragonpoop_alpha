...
namespace dragonpoop
{
    ...
    model_loader_state *model_loader_ms3d_state_make_vertex_joints::run( dpthread_lock *thd, model_loader_writelock *ml )
    {
        ...
        for( i = 0; i < e; i++ )
        {
            ...
            if( v->f.boneId >= 0 && v->f.boneId < je )
            {
                j = &( *lj )[ v->f.boneId ];
                this->makeVertexJoint( thd, ml, v, j, 1.0f / n, m );
            }
            if( v->bones[ 0 ].id >= 0 && v->bones[ 0 ].id < je && v->bones[ 0 ].weight > 0 )
            {
                j = &( *lj )[ v->bones[ 0 ].id ];
                this->makeVertexJoint( thd, ml, v, j, v->bones[ 0 ].weight / n, m );
            }
            if( v->bones[ 1 ].id >= 0 && v->bones[ 1 ].id < je && v->bones[ 1 ].weight > 0 )
            {
                j = &( *lj )[ v->bones[ 1 ].id ];
                this->makeVertexJoint( thd, ml, v, j, v->bones[ 1 ].weight / n, m );
            }
            if( v->bones[ 2 ].id >= 0 && v->bones[ 2 ].id < je && v->bones[ 2 ].weight > 0 )
            {
                j = &( *lj )[ v->bones[ 2 ].id ];
